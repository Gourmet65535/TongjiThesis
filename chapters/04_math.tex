...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{figures/fig-4-1-system-architecture.png}
  \caption{系统总体架构图}
  \label{fig:system-architecture}
\end{figure}

表现层由微信小程序构成，是用户直接交互的界面层。该层承担所有校园出行相关信息的呈现、操作输入及交互响应，包括班车信息查询、拼车信息发布与匹配、社区交流及即时消息功能。表现层通过界面组件和数据绑定将系统提供的功能直观呈现给用户，同时负责初步数据验证，如表单输入检查、日期和时间范围校验等，以减轻后端负担。通过良好的界面设计与用户体验优化，表现层实现了信息展示的直观性和操作的便捷性，使用户能够快速获取所需的出行服务。

接口层位于表现层与业务逻辑层之间，是前后端通信的桥梁。该层通过统一的RESTful API规范，将前端的请求准确路由至对应的业务模块，同时负责用户身份认证、权限校验和请求参数的初步处理，确保每一次操作均在合法范围内执行。接口层通过标准化的数据交换格式返回处理结果，既保证了前端调用的简便性，也为后端服务的独立部署和模块扩展提供了支持。接口层还承担日志记录与异常捕获功能，对请求流程进行监控，为系统维护和故障排查提供依据。
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{figures/fig-4-2-user-class-diagram.png}
  \caption{用户管理模块类图}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{figures/fig-4-3-carpool-class-diagram.png}
  \caption{拼车服务模块类图}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{figures/fig-4-4-application-sequence.png}
  \caption{拼车申请处理时序图}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/fig-4-5-matching-sequence.png}
  \caption{拼车匹配查询时序图}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{figures/fig-4-6-mysql-er.png}
...
本章对系统设计进行了全面阐述，明确了系统的总体架构、分层结构、模块划分、数据库设计及接口设计等关键内容。在总体架构方面，系统采用前后端分离的微服务架构，前端微信小程序负责用户交互和界面展示，后端Spring Boot微服务承担业务逻辑处理，并通过RESTful接口与前端通信。分层设计将系统划分为表现层、接口层、业务逻辑层及数据存储层，职责清晰、内聚性高，既保证了系统的可扩展性，也便于维护和后续功能拓展。在模块设计上，系统围绕校园出行需求，将功能划分为用户管理、拼车服务、社区交流、聊天与通知以及公共交通信息五大模块。各模块根据业务特点设计了内部逻辑和操作流程，确保核心功能的高效实现。在数据库设计方面，采用MySQL和MongoDB的混合持久化方案，分别承载结构化业务数据与内容型、消息型半结构化数据，兼顾了数据一致性与灵活扩展性，同时通过逻辑关联和应用层接口保证模块间数据的协同。接口设计遵循RESTful风格，统一路径规范和数据格式，使系统前后端交互标准化、可靠且易于扩展。

总体来看，本章通过系统架构、模块设计、数据库建模与接口设计的详细说明，为系统实现提供了清晰而完整的设计基础。各模块相互独立又通过逻辑关系协作，使校内综合交通平台在功能完整性、系统稳定性和用户体验上具备坚实保障，为系统实现提供了直接支撑。
